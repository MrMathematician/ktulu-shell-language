...
private:
  ...
  std::unordered_map<tokenType, const char*> tokenEnumStringMap; // CONST CHAR MAY CAUSE AN ERROR HERE
...
private: 
  std::unordered_map<std::string, tokenType> specialTokensMap;
...
public:  
  ...
  void addBuiltinToken(std::string tokenBuiltinName, tokenType tokenBuiltinGeneralType){
    specialTokensMap[tokenBuiltinName] = tokenBuiltinGeneralType;
  }
  ...
  tokenType getGeneralTypeIfExists(std::string tokenString){
    if(specialTokensMap.count(tokenString)){
  ...
  void displayBuiltinTokens(){
    ...
    for(const auto& pair: specialTokensMap){
      std::cout << "STRING VALUE: "<< pair.first << '\t' << "IS MAPPED WITH: ";
      std::cout << tokenClassStringMapGlobalInstance.getStringifiedEnum(pair.second) << '\n';
      currIndex ++;
    }
  }
};
...
private:
  ...
  void createAndAddOperatorTokenPhase1(){
    std::shared_ptr<AbstractTokenBuilder> tempBuilder = std::make_shared<TokenOperatorBuilder>();
  ...
  void createAndAddSeparatorTokenPhase1(){
    std::shared_ptr<AbstractTokenBuilder> tempBuilder = std::make_shared<TokenSeparatorBuilder>();
    semiTokenizedOutput.push_back(directorObject.buildToken(std::string(1, stringBuffer[counterIndexPhase1]).c_str(), getStandingLine(), tempBuilder));
  }
  ...
  void createAndAddStringTokenPhase1(){
    std::shared_ptr<AbstractTokenBuilder> tempBuilder = std::make_shared<TokenStringBuilder>();
    semiTokenizedOutput.push_back(directorObject.buildToken(tempStringPhase1.c_str(), getStandingLine(), tempBuilder));
  }
  ...
  bool returnTrueIfSeparator(){
    const char currChar = stringBuffer[counterIndexPhase1];
    if(builtInTokensData.isExists(&currChar) && 
       builtInTokensData.getGeneralTypeIfExists(&currChar) == tokenType :: SEPARATOR){
      return true;
    }
    return false;
  }

  bool returnTrueIfOperator(){
    const char currChar = stringBuffer[counterIndexPhase1];
    if(builtInTokensData.isExists(&currChar) && 
       builtInTokensData.getGeneralTypeIfExists(&currChar) == tokenType :: OPERATOR){
      return true;
    }
    return false;
  }
  ...
  void firstPhaseStartState(){
    ...
    if(stringBuffer[counterIndexPhase1] == '/'){
      createAndAddOperatorTokenPhase1();
      counterIndexPhase1 ++;
    ...
    else if(returnTrueIfSeparator()){
      createAndAddSeparatorTokenPhase1();
      counterIndexPhase1 ++;
      firstPhaseStartState();
    }
    else if(returnTrueIfOperator()){
      createAndAddOperatorTokenPhase1();
      counterIndexPhase1 ++;
      firstPhaseStartState();
    }
  ...
  void classifyKeywords(){
    for(size_t counterIndexPhase3 = 0; counterIndexPhase3 < tokenizedOutput.size(); counterIndexPhase3++){
      ...
      if(builtInTokensData.getGeneralTypeIfExists(tokenizedOutput[counterIndexPhase3] -> getTokenString()) == tokenType :: KEYWORD){
  ...
  std::shared_ptr<Token> createAndAddKeywordTokenPhase5(const char* mergedTokenString){
    std::shared_ptr<AbstractTokenBuilder> tempBuilder = std::make_shared<TokenKeywordBuilder>();
    return directorObject.buildToken(mergedTokenString, getStandingLine(), tempBuilder);
  }
  ...
  std::shared_ptr<Token> mergeIfApplies(std::shared_ptr<Token> tokenPtr1, std::shared_ptr<Token> tokenPtr2){
    std::shared_ptr<Token> mergedToken = nullptr;

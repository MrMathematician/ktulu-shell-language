...
class BraceAndBracketChecker{
  std::shared_ptr<Token> checkBalanceOfBracket(std::shared_ptr<Script> scriptToCheck){
    ...
    for(int i = 0; i < scriptToCheck -> get_token_vector_pointer() -> size(); i++){
...
private: 
  ...
  */
  ...
  //std::string actionToken; // FOR MAJOR EXPRESSIONS
  //std::string literalToken; // FOR MINOR EXPRESSIONS (STATEMENTS) [SPECIFICALLY FOR REDEFINING VARIABLES]
  //
